# app.py — Club Fit Finder (Attackers)
...
st.set_page_config(page_title="Club Fit — Attackers", layout="wide")
st.title("🏟️ Club Fit Finder — Attackers")
...
if df is None:
    ...
    if not uploaded:
        st.warning("Add WORLDJUNE25.csv to your repo or upload it here.")
        st.stop()
...
# Weights (from your attacker notebook)
weight_factors = {
    'xG per 90': 2,
    'Dribbles per 90': 2,
    'Crosses per 90': 2,
    'Non-penalty goals per 90': 1,
    'xA per 90': 1,
...
league_strengths = {
    ...
    'Netherlands 2.':37.14,'Switzerland 2.':45.00,'Iceland 1.':34.29,'Ireland 1.':34.29,'Sweden 2.':34.29,
    ...
    'Northern Ireland 1.':11.43,'England 5.':11.43,'Andorra 1.':10.00,'Estonia 1.':8.57,'England 10.':5.00,
    'Scotland 3.':5.00,'England 6.':5.00
}

DEFAULT_LEAGUE_WEIGHT = 0.1
DEFAULT_MARKET_WEIGHT = 0.1

# Attacker position filter from your notebook
PREFIXES = ('RWF', 'LWF', 'LAMF', 'RAMF', 'AMF', 'RW, ', 'LW, ')
def position_is_attacker(pos: str) -> bool:
...
with st.sidebar:
    ...
    leagues_available = sorted(set(included_leagues) | set(df.get('League', pd.Series([])).unique()))
    leagues_selected = st.multiselect("Included leagues", leagues_available, default=included_leagues)

    # Build pool restricted to attackers for target selection
    pool = df[df['League'].isin(leagues_selected)].copy()
    pool = pool[pool['Position'].apply(position_is_attacker)]

    if pool.empty:
        st.error("No attacker records found for the chosen leagues.")
        st.stop()

    target_player = st.selectbox("Target player (attackers only)", sorted(pool['Player'].dropna().unique()))

    # Minutes & Age
    min_minutes, max_minutes = st.slider("Minutes filter (players used to build team profiles)",
                                         0, 5000, (500, 5000))
    # Age slider based on dataset
    age_min_all = int(pd.to_numeric(df.get('Age', pd.Series([])), errors='coerce').min() or 14)
    age_max_all = int(pd.to_numeric(df.get('Age', pd.Series([])), errors='coerce').max() or 45)
    min_age, max_age = st.slider("Age filter", age_min_all, age_max_all, (16, 33))
    ...
    # Market value override for target
    manual_override = st.number_input("Target market value override (€)", min_value=0, value=0, step=100_000)

    # Other options
    exclude_current = st.checkbox("Exclude current team from results", True)
    top_n = st.number_input("Show top N teams", 5, 100, 25, 5)
...
# ---------- Filter dataset (attackers only) ----------
df_f = df[df['League'].isin(leagues_selected)].copy()
df_f = df_f[df_f['Position'].apply(position_is_attacker)]
...
# ---------- Build club profiles (team means over ATTACKERS only) ----------
club_profiles = df_f.groupby('Team')[features].mean().reset_index()

# Map each team to its dominant league and average team market value (within attacker pool)
...
# ---------- Similarity (scaled features + weights) ----------
...
dist = np.linalg.norm((scaled_team - target_scaled) * weights, axis=1)
rng = dist.max() - dist.min()
base_fit = (1 - (dist - dist.min()) / (rng if rng > 0 else 1)) * 100.0
...
# ---------- Optional: drop current team & dedupe teams ----------
if exclude_current:
    club_profiles = club_profiles[club_profiles['Team'] != target_row['Team']]
...
st.download_button("⬇️ Download all results (CSV)", data=csv, file_name="club_fit_attackers.csv", mime="text/csv")
